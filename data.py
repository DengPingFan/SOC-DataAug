...
def cv_random_flip(img, label, prob=0.5):
...
def randomCrop(image, label, border=0.1):
    image_width, image_height = image.size[0], image.size[1]
    if border < 1:
        border = int(min(image_width, image_height) * border)
...
def randomRotation(image, label, prob=0.2, angle=15):
    mode = Image.BICUBIC
    if random.random() < prob:
...
def colorEnhance(image, range_brightness=(5, 15), range_contrast=(5, 15), range_color=(0, 20), range_sharpness=(0, 30)):
    bright_intensity = random.randint(*range_brightness) / 10.0
    image = ImageEnhance.Brightness(image).enhance(bright_intensity)
    contrast_intensity = random.randint(*range_contrast) / 10.0
    image = ImageEnhance.Contrast(image).enhance(contrast_intensity)
    color_intensity = random.randint(*range_color) / 10.0
    image = ImageEnhance.Color(image).enhance(color_intensity)
    sharp_intensity = random.randint(*range_sharpness) / 10.0
...
def randomPeper(img, prob_noise=0.0015, prob_turn=0.5):
    img = np.array(img)
    noiseNum = int(prob_noise * img.shape[0] * img.shape[1])
